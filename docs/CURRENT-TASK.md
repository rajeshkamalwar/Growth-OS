...
## Required Outcome
- Add one additive migration and SQLAlchemy model for a bounded `workspace_goals` table with
...
- Enforce the workspace's same-tenant ownership through service checks and a composite database
  foreign key. Missing and cross-tenant workspaces/goals both return `not_found`.
- Expose `POST`, `GET`, and `PATCH` at
  `/api/v1/tenants/{tenant_id}/workspaces/{workspace_id}/goal`; duplicate creation returns the
  established structured conflict response.
- Commit create/update atomically with exactly one append-only `workspace_goal.created` or
  `workspace_goal.updated` audit event. Audit details contain only `workspace_id` and sorted
...
- Do not change authentication, authorization, tenant-context boundaries, billing, secrets,
  ...
  product/architecture/goal/decision documents.
